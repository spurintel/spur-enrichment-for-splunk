...
class SpurIPLocation(StreamingCommand):
    ...
    def stream(self, records):
        ...
        if not GEOIP_LIB:
            logger.error("Neither geoip2 nor maxminddb is installed.")
            for record in records:
                empty_data = create_empty_location_data()
                record = populate_record_with_location_data(
                    record, empty_data, selected_fields, "No geoip2 or maxminddb library installed"
                )
                record = ensure_selected_fields_present(record, selected_fields)
                yield record
            return
            
        if not os.path.exists(MMDB_PATH):
            logger.error("MMDB file not found at %s", MMDB_PATH)
            for record in records:
                empty_data = create_empty_location_data()
                record = populate_record_with_location_data(
                    record, empty_data, selected_fields,
                    f"MMDB file not found at {MMDB_PATH}, please configure a data input for the Spur IPGeo MMDB"
                )
                record = ensure_selected_fields_present(record, selected_fields)
                yield record
            return
